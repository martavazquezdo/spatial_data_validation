...
def build_execution_summary(
    ...
    extraction_global_summary_df: pd.DataFrame | None = None,
) -> pd.DataFrame:
    """
    ...
    extraction_global_summary_df : DataFrame | None
    ...
    n_tags_error = np.nan
    if extraction_global_summary_df is not None and not extraction_global_summary_df.empty:
        row = extraction_global_summary_df.iloc[0]
...
def generate_execution_report_text(
    ...
    lines.append(f"- Records ready for spatial validation: {summary['n_ready_for_spatial_validation']}")
